...
async function renderCurCard(sharp, opts){
  ...
  try{
    if(isTermux){
      // Termux: niente blur (pesante e a volte fallisce), solo tinta scura solida
  ...
  const ff = isTermux ? 'DejaVu Sans' : 'sans-serif';
  const svg = isTermux ? `
<svg width="900" height="480" viewBox="0 0 900 480" xmlns="http://www.w3.org/2000/svg">
  <rect x="0" y="0" width="900" height="480" rx="38" fill="#14141A" stroke="#2a2a34" stroke-width="2"/>
  <circle cx="328" cy="62" r="6" fill="${isNowPlaying ? '#1DB954' : '#888'}" />
  <text x="342" y="67" font-family="${ff}" font-size="14" fill="${isNowPlaying ? '#1DB954' : '#aaa'}" font-weight="700">${esc(isNowPlaying ? 'IN RIPRODUZIONE' : 'ULTIMO ASCOLTO')}</text>
  <text x="328" y="112" font-family="${ff}" font-size="40" fill="#FFFFFF" font-weight="700">${esc(tName)}</text>
  <text x="328" y="148" font-family="${ff}" font-size="20" fill="#FFFFFF" font-weight="700">${esc(tArtist.toUpperCase())}</text>
  <g>
    <rect x="328" y="212" width="250" height="78" rx="18" fill="#2a2a34" stroke="#3a3a44"/>
    ...
    <rect x="600" y="212" width="250" height="78" rx="18" fill="#2a2a34" stroke="#3a3a44"/>
    ...
    <rect x="328" y="308" width="250" height="78" rx="18" fill="#2a2a34" stroke="#3a3a44"/>
    ...
    <rect x="600" y="308" width="250" height="78" rx="18" fill="#2a2a34" stroke="#3a3a44"/>
