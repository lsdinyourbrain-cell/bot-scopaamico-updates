...
const isAntinukeWhitelisted = (cfg, jid, alt) => {
    if (!cfg?.whitelist?.length) return false;
    const checkOne = (x) => {
        if (!x) return false;
        const norm = toDigits(x);
        if (!norm || norm.length < 5) return false;
        return cfg.whitelist.some(w => {
            const wn = toDigits(w);
            if (!wn || wn.length < 5) return false;
            // match esatto o suffisso (gestisce prefisso paese mancante/presente)
            // evita match troppo permissivo su numeri corti: richiedi almeno 5 cifre coincidenti
            return norm === wn || norm.endsWith(wn) || wn.endsWith(norm);
        });
    };
    // Controlla jid principale e alt (PN/LID) — copre LID vs PN
    if (checkOne(jid)) return true;
    if (alt && checkOne(alt)) return true;
    // LID spesso è casuale: se jid è @lid e alt è PN, l'alt è già controllato sopra;
    // fallback esplicito per robustezza (es. whitelist contiene PN, jid è LID)
    if (String(jid||'').endsWith('@lid') && alt && String(alt).includes('@s.whatsapp.net')) return checkOne(alt);
    // Viceversa: jid è PN ma whitelist potrebbe contenere LID storico (poco probabile ma gestito)
    if (String(alt||'').endsWith('@lid') && jid && String(jid).includes('@s.whatsapp.net')) return checkOne(jid);
    return false;
};
