...
const askAI = async ({ services, system, user, maxTokens = 800 }) => {
    ...
    const activeKey = (db?._ai?.apiKey) || AI_API_KEY || (process.env.AI_API_KEY||'').trim();
    console.log(`[AI] key len=${String(activeKey||'').length} url=${AI_API_URL} model=${AI_MODEL} from=${db?._ai?.apiKey?'db':'env/config'}`);
