...
// `mult` moltiplica l'XP guadagnato (eventi tipo "Doppio XP").
const grantXp = (user, messages = 1, mult = 1) => {
    const userXp = Number.isFinite(user.xp) ? user.xp : 0;
    const gain = Math.round(XP_PER_MSG * Math.max(1, messages) * Math.max(0.1, mult));
    user.xp = userXp + gain;
