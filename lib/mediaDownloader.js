...
const runYoutubeDl = async (url, flags) => {
    ...
    try {
        ...
        // continuazione). Aggiorniamo il binario e ritentiamo UNA volta:
        // se il problema era la versione, il secondo giro va a buon fine.
        const forbidden = /http error 403|403 forbidden|forbidden/i.test(message);
        if (forbidden) {
            console.warn('[yt-dlp] 403: aggiorno yt-dlp e ritento il download.');
            try {
