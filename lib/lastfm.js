...
// Ritorna { playcount, listeners, userplaycount, duration } (numeri, 0 se assenti).
const getTrackInfo = async (artist, track, username) => {
    ...
    const t = data?.track;
    if (!t) return { playcount: 0, listeners: 0, userplaycount: 0, duration: 0 };
    return {
        playcount: Number(t.playcount) || 0,
        listeners: Number(t.listeners) || 0,
        userplaycount: Number(t.userplaycount) || 0,
        duration: (() => {
            let dur = Number(t.duration) || 0;
            // Last.fm a volte ritorna i millisecondi invece dei secondi:
            // 209000 (ms) per una canzone di 3:29. Se il valore è > 27 ore,
            // lo convertiamo in secondi.
