...
class Archiver {
    ...
    appendMessaggio(jid, msg) {
        ...
        try {
            ...
            // Riga leggibile immediata: il dump periodico rigenera comunque chat.txt.
            const data = new Date(rec.ts * 1000).toLocaleString('it-IT');
            const chi = rec.fromMe ? '🤖' : (rec.pushName || rec.from);
            fs.appendFileSync(path.join(folder, 'chat.txt'), `[${data}] ${chi}: ${rec.testo || `(${rec.tipo})`}\n`, 'utf-8');
            return true;
        } catch (e) {
