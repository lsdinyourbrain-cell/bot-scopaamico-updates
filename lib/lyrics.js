...
// Formato atteso da .cur: "Artista - Titolo" (esatto Last.fm). Non sovrascrivere se già preciso.
const searchLyrics = async (axios, query) => {
    const q = String(query || '').trim();
    if (!q) return null;

    // Se query è già "Artista - Titolo", usala diretta (evita iTunes che prende sbagliato)
    let artist = '';
    let title = q;
    const dash = q.indexOf(' - ');
    if (dash > 2 && q.length - dash > 2) {
        artist = q.slice(0, dash).trim();
        title = q.slice(dash + 3).trim();
    } else if (q.includes(' - ')) {
        const p = q.split(' - ');
        if (p.length >= 2) { artist = p[0].trim(); title = p.slice(1).join(' - ').trim(); }
    }
    // Solo se non abbiamo artista/titolo separati, prova iTunes come fallback (verifica match)
    if (!artist || !title || artist === q) {
        try {
            ...
            if (song) {
                // Usa iTunes solo se matcha almeno in parte (evita canzone sbagliata)
                const norm = s=> String(s||'').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').trim();
                const itTitle = norm(song.trackName), itArtist = norm(song.artistName);
                const qNorm = norm(q);
                if (itTitle && qNorm.includes(itTitle.slice(0,4)) || itArtist && qNorm.includes(itArtist.slice(0,4))) {
