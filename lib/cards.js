...
const cardSvg = ({ set, card, entry }) => {
  ...
  <text x="${CARD_W / 2}" y="${CARD_H - 39}" font-family="Arial, sans-serif" font-size="14" fill="#c8c8e0" text-anchor="middle">VEX BOT · ${esc(set.key)} · ${entry ? 'POSSIEDITA' : ''}</text>
</svg>`;
};
...
const packSvg = () => `<svg width="${360}" height="${500}" xmlns="http://www.w3.org/2000/svg">
  ...
  <text x="180" y="93" font-family="Arial, sans-serif" font-size="26" font-weight="bold" fill="#ffffff" text-anchor="middle">VEX BUSTA</text>
...
const renderPack = async (sharp) => svgToPng(packSvg(), sharp);
