...
const runPower = async (sock, msg, args, context, cfg) => {
    ...
    const refuseIfInvalidTarget = (target) => {
        if (botSelf.some(b => sameJid(target, b))) {
            reply(`${cfg.emoji} *_NO._*\n${SEP}\n▸ Il bot non fa questo\n  genere di cose.\n▸ Trovati un umano. 😌`);
            return true;
        }
...
◈ Vex Bot`;
    ...
    if (isButton) {
        ...
        if (!hasLevel) {
            ...
            if (againTarget) {
                if (refuseIfInvalidTarget(againTarget)) return;
        ...
        clearPending();
        if (refuseIfInvalidTarget(target)) return;
...
◈ Vex Bot`;
    ...
    if (refuseIfInvalidTarget(rawTarget)) return;
