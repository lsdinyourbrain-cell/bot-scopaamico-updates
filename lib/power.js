...
const runPower = async (sock, msg, args, context, cfg) => {
    ...
    if (isButton) {
        ...
        await sock.sendMessage(from, { text: intro, mentions: target ? [sender, target] : [sender] });
        if (typeof sleep === 'function') await sleep(900);
        await sock.sendMessage(from, { text: phraseMsg, mentions: target ? [sender, target] : [sender] });
        if (typeof sleep === 'function') await sleep(1100);
        await sock.sendMessage(from, { text: `_${outro}_`, mentions: [sender] });
        return;
    }
