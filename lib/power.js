...
//  Mittente e vittima vengono evidenziati via @mentions nell'invio (v. runPower).
// ─────────────────────────────────────────────────────────────────────────────
...
// Citazioni ironiche finali: dopo la frase si manda un messaggio di commento
// "a telecamera spenta", sempre sopra le righe ma da ridere.
const OUTRO = {
    main: [
        "Il resto della scena è\ncensurato: siamo in chat,\nnon a Sanremo.",
        "Se qualcuno ha registrato,\nlo ammetta ora, che ne ha\nil coraggio.",
        "Privacy: off. Vergogna:\non. Peccato: veniale.",
        "Il gruppo ha chiesto il bis,\nma il bis resta in\narchivio segreto.",
        "La segnalazione è partita.\nA chi tocca ora, tocca.",
        "Il resoconto finisce qui:\nil resto è roba da\nprocesso di famiglia.",
    ],
    scopa: [
        "I vicini hanno chiamato\ni pompieri per controllare\nche il letto fosse sano.",
        "Il letto ha chiesto un\naumento di stipendio e\nla pensione anticipata.",
        "Consiglio spassionato:\nfai dare un'occhiata alla\nschiena da un chiropratico.",
    ],
    sborra: [
        "Il condominio ha organizzato\nuna raccolta fondi per\nl'idraulico.",
        "Il contatore dell'acqua ha\nchiesto il trasferimento in\nun altro comune.",
        "La maglietta di Y è stata\nricoverata con un pronto\nsoccorso.",
    ],
    sega: [
        "Il polso ha chiesto\nferie compensate e un\nmonumento funebre.",
        "Il braccio è entrato in\nsciopero: rivuole il suo\ncontratto nazionale.",
        "Il vicino credeva fosse\nun cantiere: ha chiesto\ni danni.",
    ],
    ditalino: [
        "Il dito ha iniziato una\ncarriera da terremotologo:\nha predetto tutto.",
        "Y ha chiesto un diploma\ndi resistenza con lode\nper il momento storico.",
        "Il dito è stato dichiarato\npatrimonio nazionale:\na regime protegge Y.",
    ],
    squirt: [
        "Il comune ha decretato\nlo stato di allerta idrica:\nl'acqua doveva finire\nproprio lì.",
        "Il pavimento ha chiesto\nclass action per via dei\ndanni ricevuti.",
        "Y ora parla solo con la\nmaschera da snorkeling:\nper sicurezza.",
    ],
};

// Esegue il flusso power per un comando. cfg = { command, emoji, title }.
const runPower = async (sock, msg, args, context, cfg) => {
    const { command, textArgs, from, sender, senderAlt, targetJid, isButton, reply, services } = context;
    ...
    // Numero "leggibile": in LID mode il PN alternativo evita @lid strani.
    ...
    if (isButton) {
        ...
        const frase = rnd
            .replace(/X/g, `@${senderShow}`)
            .replace(/Y/g, target ? `@${disp(target)}` : '@nessuno');
        ...
        // ── FINE FESTA: tre messaggi consecutivi, molto ironici ────────────
        // 1) il bot "prepara" la potenza, 2) la frase action, 3) la battuta.
        const intro = `${cfg.emoji} *${cfg.title}*\n━━━━━━━━━━━━━━━━━━\n⚡ @${senderShow} ha scelto la\npotenza *${meta.label}* (${meta.tag})\n💢 obiettivo: @${targetName}\n🔁 Preparazione in corso...`;
        const phraseMsg = `_💬 ${frase}_`;
        const outro = randomChoice((OUTRO[cfg.command] && OUTRO[cfg.command].length ? OUTRO[cfg.command] : OUTRO.main));
        ...
        if (typeof sleep === 'function') await sleep(1100);
    ...
    await sendButtons(sock, from,
        `⚡ ${cfg.emoji} *${cfg.title}* con @${disp(targetJid)}\n\nScegli la POTENZA della tua azione 👇`,
        [
