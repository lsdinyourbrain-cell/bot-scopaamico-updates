...
// ── Carica PHRASES da phrases/*.txt se presenti ──────────────────────────
try {
    const phrasesLib = require('./phrases');
    for (const cmd of Object.keys(PHRASES)) {
        for (const lvl of Object.keys(PHRASES[cmd])) {
            const fromFile = phrasesLib.getPhrases(`${cmd}_${lvl}`);
            if (fromFile) PHRASES[cmd][lvl] = fromFile;
        }
    }
} catch (_) {}
...
try {
    const phrasesLib = require('./phrases');
    for (const k of Object.keys(OUTRO)) {
        const fromFile = phrasesLib.getPhrases(`outro_${k}`);
        if (fromFile) OUTRO[k] = fromFile;
    }
} catch (_) {}

// Estrae un JID da un argomento tipo "@39333..." o "39333..." (per gli id
// dei pulsanti, che NON portano menzioni reali).
const jidFromArg = (t) => {
    const m = String(t || '').match(/(\d{7,})/);
...
const runPower = async (sock, msg, args, context, cfg) => {
    ...
    // Rifiuti comuni: il bot e se stessi non sono bersagli validi
    const refuseIfInvalidTarget = async (target) => {
        if (botSelf.some(b => sameJid(target, b))) {
            await reply(`${cfg.emoji} *_NO._*\n${SEP}\n▸ Il bot non fa questo\n  genere di cose.\n▸ Trovati un umano. 😌`);
            return true;
        }
        if (sameJid(target, sender)) {
            await reply(`🪞 *_DA SOLI?_*\n${SEP}\n▸ Tagga qualcuno ALTRO:\n▸ _.${cfg.command} @nome_`);
            return true;
        }
        return false;
    };

    // Mostra la schermata di scelta potenza per un bersaglio
    const showPowerSelection = async (rawTarget) => {
        if (!db[from]) db[from] = {};
        if (!db[from][key]) db[from][key] = {};
        db[from][key][pendKey] = { sender, target: rawTarget, command: cfg.command, ts: Date.now() };
        saveDB();

        const targetDisp = disp(rawTarget);
        const selText =
`◈  ${toBold(cfg.title)}  ◈
${SEP}
...
${SEP}
⏳ _Hai 60 secondi per scegliere_
◈ Vex Bot`;
        ...
        ], msg, [senderMention, rawTarget], { headerTitle: `${cfg.emoji} ${toBold(cfg.title)}`, footerText: '⬇️ Scegli la potenza' });
    };

    // ── PRESSIONE PULSANTE ─────────────────────────────────────────────
    if (isButton) {
        const level = parseInt(textArgs, 10);
        const hasLevel = [1, 2, 3].includes(level);

        // Bottone "👤 Altra vittima" (nessun livello):
        //  • se l'id contiene un @numero è il pulsante "🔄 Ancora" → riapre
        //    la scelta potenza sullo stesso bersaglio;
        //  • altrimenti invita a taggare qualcun altro invece di restare muto.
        if (!hasLevel) {
            const againTarget = jidFromArg(textArgs);
            if (againTarget) {
                if (await refuseIfInvalidTarget(againTarget)) return;
                await showPowerSelection(againTarget);
                return;
            }
            clearPending();
            return reply(`👤 *${toBold(cfg.title)} — NUOVA VITTIMA*\n${SEP}\n▸ Tagga chi vuoi colpire:\n▸ _.${cfg.command} @nome_`);
        }

        const pending = db[from]?.[key]?.[pendKey];
        const argTarget = jidFromArg(textArgs); // target dentro l'id del pulsante

        let target = null;
        if (pending && sameJid(sender, pending.sender)) {
            // TIMEOUT VISIBILE: il pending scade dopo 60s e lo diciamo
            if (Date.now() - pending.ts > pendingTTL) {
                clearPending();
                return reply(`⏳ *TEMPO SCADUTO*\n${SEP}\n▸ Hai *60 secondi* per scegliere\n  la potenza.\n▸ Ripeti: _.${cfg.command} @nome_`);
            }
            target = pending.target;
        } else if (argTarget) {
            // Bottone "Ancora": il target era nell'id (@numero)
            target = argTarget;
        }
        if (!target) return; // pressione senza contesto: ignora in silenzio

        clearPending();
        if (await refuseIfInvalidTarget(target)) return;
        ...
        const frase = rnd.replace(/X/g, `@${senderShow}`).replace(/Y/g, `@${disp(target)}`);
...
◈ _Vex Bot_`;
    ...
    // ── COMANDO TESTUALE ───────────────────────────────────────────────
    const rawTarget = targetJid || jidFromArg(textArgs);
    if (!rawTarget) return reply(`🎯 *${toBold(cfg.title)}*\n${SEP}\n▸ Tagga chi vuoi colpire:\n▸ ${toBold('.' + cfg.command + ' @nome')}`);
    if (await refuseIfInvalidTarget(rawTarget)) return;

    await showPowerSelection(rawTarget);
};
