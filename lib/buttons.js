...
const sendButtons = async (sock, from, text, buttons, quoted, mentions) => {
    ...
    if (!list.length || tooLong) {
        ...
        const plain = { text: cleanText };
        if (resolved?.length) plain.mentions = resolved;
        return sock.sendMessage(from, plain, { quoted });
    }

    try {
        const content = buildInteractiveContent(cleanText, list, resolved);
...
const editButtons = async (sock, from, text, buttons, editKey, quoted, mentions) => {
    ...
    try {
        ...
        const resolved = mentions ? await resolveMentions(from, mentions) : null;
        const content = {
            ...buildInteractiveContent(cleanText, list, resolved),
            edit: {
...
module.exports = { sendButtons, editButtons, sendButtonsWithKey, sendCarousel, buttonRegistry, stripEmoji, normalizeBtnText, BTN_EMOJI_STRIP, BTN_REGISTER_TTL, setMentionResolver };
