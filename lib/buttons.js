...
const sendButtons = async (sock, from, text, buttons, quoted) => {
    ...
    try {
        ...
        const now = Date.now();
        for (const b of list) {
            buttonRegistry.set(`${from}|${stripEmoji(b.label).toLowerCase()}`, { id: String(b.id || b.label), ts: now });
        }
        return true;
    } catch (e) {
...
module.exports = { sendButtons, buttonRegistry, stripEmoji, BTN_EMOJI_STRIP, BTN_REGISTER_TTL };
