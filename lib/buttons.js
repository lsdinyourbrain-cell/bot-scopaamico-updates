...
// Necessario perché i messaggi interactive partono con relayMessage (non passa
// dal wrapper di sock.sendMessage, che invece risolve già le mentions).
...
// Riscrive nel testo i token @<numero> (lid o PN) con il numero della mention
// RISOLTA: così il testo coincide con mentionedJid e WhatsApp evidenzia il tag
// per davvero. Serve al percorso relayMessage dei pulsanti, che NON passa dal
// wrapper di sock.sendMessage (che ha già la sua riscrittura).
const rewriteTagText = (text, origMentions, resolvedMentions) => {
    ...
    for (let i = 0; i < origMentions.length; i++) {
        ...
        // (?![0-9]) evita di sostituire @123 dentro @1234
...
// I pulsanti NON scadono più nella pratica: 30 giorni. Il registro viene
// comunque potato periodicamente (v. pruneButtonRegistry) così la memoria
// resta sotto controllo anche dopo settimane di uso intenso.
const BTN_REGISTER_TTL = 30 * 24 * 60 * 60 * 1000; // 30 giorni

// Contatore per potare il registro di rado (ogni N pulsanti registrati),
// evitando di scansionare tutta la Map a ogni invio.
...
// `mentions` (opzionale) viene messo in interactiveMessage.contextInfo:
// generateWAMessageFromContent lo fonde con le info della citazione, così le
// menzioni taggano anche dentro i messaggi interactive.
const buildInteractiveContent = (text, list, mentions) => {
...
const registerButtons = (from, list) => {
    const now = Date.now();
    // Ogni tanto pulisci le voci più vecchie del TTL (i pulsanti non scadono,
    // ma la memoria sì che va tenuta sotto controllo).
...
// `mentions` (opzionale, 6° parametro): array di jid taggati nel messaggio.
const sendButtons = async (sock, from, text, buttons, quoted, mentions) => {
    ...
    const resolved = mentions ? await resolveMentions(from, mentions) : null;
    // Il testo deve mostrare i numeri delle mentions RISOLTE (mai @lid casuali)
    const finalText = mentions ? rewriteTagText(cleanText, mentions, resolved) : cleanText;
    ...
    try {
        const content = buildInteractiveContent(finalText, list, resolved);
...
const editButtons = async (sock, from, text, buttons, editKey, quoted, mentions) => {
    const list = (buttons || []).slice(0, 3);
    const cleanText = String(text || '').trim();

    const tooLong = cleanText.length > 1024 || Buffer.byteLength(cleanText, 'utf8') > 1024;
    if (!editKey?.id || !list.length || tooLong) {
        return sendButtons(sock, from, text, buttons, quoted, mentions);
    }
    ...
    try {
        ...
        const finalText = mentions ? rewriteTagText(cleanText, mentions, resolved) : cleanText;
        const content = {
            ...buildInteractiveContent(finalText, list, resolved),
            edit: {
...
// Come sendButtons ma ritorna la key (id) del messaggio inviato, così il
// chiamante può eliminarlo o modificarlo dopo (es. i pulsanti di movimento del
// labirinto, che cambiano a ogni mossa). In caso di errore torniamo al testo
// semplice e ritorniamo la sua key (o null se fallisce del tutto).
const sendButtonsWithKey = async (sock, from, text, buttons, quoted, mentions) => {
    const list = (buttons || []).slice(0, 3);
    const cleanText = String(text || '').trim();
...
const sendCarousel = async (sock, from, opts, quoted) => {
    ...
    // Oltre all'URL si può passare card.imageBuffer (un Buffer PNG già pronto,
    // es. i labirinti renderizzati del comando .labirinto).
    ...
    // WhatsApp RIFIUTA il carosello ("messaggio non supportato") se una card
    // non ha un'immagine: teniamo solo le card con media allegata. Se non ne
    // resta nessuna, ritorniamo false e il chiamante può usare un fallback.
...
module.exports = { sendButtons, editButtons, sendButtonsWithKey, sendCarousel, buttonRegistry, stripEmoji, normalizeBtnText, BTN_EMOJI_STRIP, BTN_REGISTER_TTL, setMentionResolver, rewriteTagText };
