                    ...
                    if (b.type === 'cta_url' || b.url) {
                        return proto.Message.InteractiveMessage.NativeFlowMessage.NativeFlowButton.create({
                            ...
                            buttonParamsJson: JSON.stringify({
                                ...
                                merchant_url: String(b.url || b.link || ''),
                            }),
                        });
                    }
                    return proto.Message.InteractiveMessage.NativeFlowMessage.NativeFlowButton.create({
...
const sendButtons = async (sock, from, text, buttons, quoted, mentions, opts) => {
    let _t = String(text || '').trim();
    if (_t && !_t.includes('⋆｡˚') && !_t.includes('╰⭒')) {
        let b = _t.replace(/◈\s*_Vex Bot_\s*/gi,'').trim();
        b = b.split('\n').map(l=>{ const s=l.trim(); if(/^[━─═━┈╌─]+$/.test(s)||/^◈/.test(s)) return ''; return l; }).join('\n').replace(/\n{3,}/g,'\n\n').trim();
        if (b) {
            let ttl = 'VEX';
            const m1=b.match(/^\s*[^\n]*\*([^*]{2,15})\*/);
            if(m1){ const c=m1[1].replace(/[_*`]/g,'').trim().toUpperCase().slice(0,12); if(c) ttl=c; }
