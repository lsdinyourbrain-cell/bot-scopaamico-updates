...
const { toDarkFont } = require('./font');

/**
 * Dark-theme wrapper: converte un testo in Fraktur Bold.
 * Mantiene intatti emoji, numeri, simboli e markup WhatsApp.
 * Si affida al proxy su sock.sendMessage per il fallback testuale.
 */
const style = (s) => toDarkFont(String(s || '').trim());
    ...
    return {
        interactiveMessage: proto.Message.InteractiveMessage.create({
            ...
            footer: proto.Message.InteractiveMessage.Footer.create({
                text: '⬇️ ' + style('Premi un pulsante per eseguire'),
            }),
            nativeFlowMessage: proto.Message.InteractiveMessage.NativeFlowMessage.create({
                buttons: list.map(b => {
                    ...
                    if (b.type === 'single_select') {
                        ...
                        const params = {
                            title: style(String(b.title || b.label || 'Scegli')).slice(0, 50),
                            sections: [{
                                title: style(String(b.sectionTitle || '')).slice(0, 50),
                                rows: rows.map(r => ({
                                    header: style(String(r.header || '')).slice(0, 30),
                                    title: style(String(r.title || '')).slice(0, 60),
                                    description: style(String(r.description || '')).slice(0, 120),
                    ...
                    if (b.type === 'copy') {
                        return proto.Message.InteractiveMessage.NativeFlowMessage.NativeFlowButton.create({
                            ...
                            buttonParamsJson: JSON.stringify({
                                display_text: style(String(b.label)).slice(0, 30),
    ...
    for (const b of list) {
        ...
        if (b.type === 'single_select') {
            for (const r of (b.rows || [])) {
                ...
                const normTitle = normalizeBtnText(r.title || r.header || '');
                const normHeader = normalizeBtnText(r.header || '');
                const normDarkTitle = normalizeBtnText(style(r.title || ''));
                const normDarkHeader = normalizeBtnText(style(r.header || ''));
                buttonRegistry.set(`${from}|${normTitle}`, entry);
                buttonRegistry.set(`${from}|${normDarkTitle}`, entry);
                buttonRegistry.set(`${from}|${normDarkHeader}`, entry);
        ...
        const normLabel = normalizeBtnText(b.label || b.id || '');
        const normDarkLabel = normalizeBtnText(style(b.label || ''));
        buttonRegistry.set(`${from}|${normLabel}`, entry);
        buttonRegistry.set(`${from}|${normDarkLabel}`, entry); // label in dark font
...
const sendButtonsWithKey = async (sock, from, text, buttons, quoted) => {
    const list = (buttons || []).slice(0, 3);
    const cleanText = String(text || '').trim();

    const tooLong = cleanText.length > 1024 || Buffer.byteLength(cleanText, 'utf8') > 1024;
    if (!list.length || tooLong) {
        const sent = await sock.sendMessage(from, { text: cleanText }, { quoted });
...
module.exports = { sendButtons, editButtons, sendButtonsWithKey, sendCarousel, buttonRegistry, stripEmoji, normalizeBtnText, BTN_EMOJI_STRIP, BTN_REGISTER_TTL };
