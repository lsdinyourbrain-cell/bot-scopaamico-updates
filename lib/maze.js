...
const stepMaze = async ({ sock, from, sender, raw, db, saveDB, getUser, sharp, quoted }) => {
    ...
    const btnKey = await sendButtonsWithKey(sock, from, MOVES_TEXT, [moveNavButton(), { label: '❌ Termina', id: 'labirinto termina' }, { label: '🔄 Nuova partita', id: 'labirinto termina' }], quoted);
    if (g.btnKey) { try { await sock.sendMessage(from, { delete: g.btnKey }); } catch (_) {} }
