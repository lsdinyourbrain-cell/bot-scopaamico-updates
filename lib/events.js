...
//    raduno         → ogni partecipante riceve soldi (durata 2-10 min)
// ─────────────────────────────────────────────────────────────────────────────
...
const EVENT_TYPES = {
    ...
    raduno:         { emoji: '👥', label: 'Raduno',           defaultMin: 5,   minMin: 2, maxMin: 10, desc: 'Ogni partecipante riceve soldi (durata 2-10 min)' },
};
...
const start = (db, chatId, type, minutes) => {
    ...
    const minDur = meta.minMin || 5;
    const maxDur = meta.maxMin || 1440;
    const dur = Math.min(maxDur, Math.max(minDur, minutes || meta.defaultMin));
    db._events[chatId][type] = { until: Date.now() + dur * MIN_MS, startedAt: Date.now() };
    return { ok: true, dur, meta };
};

// Evento casuale: sceglie un tipo a caso e una durata (rispettando i limiti
// del tipo, es. Raduno → 2-10 min). Usato da .evento random (owner/admin).
const startRandom = (db, chatId) => {
    const types = Object.keys(EVENT_TYPES);
    const type = types[Math.floor(Math.random() * types.length)];
    const meta = EVENT_TYPES[type];
    const minDur = meta.minMin || 5;
    const maxDur = meta.maxMin || 120;
    const dur = minDur + Math.floor(Math.random() * (maxDur - minDur + 1));
    return start(db, chatId, type, dur);
};

// Raduno: ogni utente che partecipa (manda un messaggio) riceve soldi UNA
// volta per evento. Il totale e i già-partecipanti vivono in db._raduno.
const radunoReward = (db, chatId) => {
    if (!db._raduno) db._raduno = {};
    if (!db._raduno[chatId]) {
        db._raduno[chatId] = { amount: 100 + Math.floor(Math.random() * 151), done: {} };
    }
    return db._raduno[chatId];
};

const participateRaduno = (db, chatId, sender) => {
    if (!isActive(db, chatId, 'raduno')) return null;
    const r = radunoReward(db, chatId);
    if (r.done[sender]) return null;
    r.done[sender] = true;
    return r.amount;
};
...
module.exports = {
    EVENT_TYPES, isActive, start, stop, remainingMin, activeList, startRandom,
    spawnBoss, bossShot, startRain, takeRain, openCassa,
    radunoReward, participateRaduno,
};