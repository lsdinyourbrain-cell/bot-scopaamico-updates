...
// ── IMAGE SEARCH & SEND (Google accurato + verifica piedi) ───────────────────
const searchImages = async (query, limit=3) => {
    try {
        const q = encodeURIComponent(query);
        const isFeet = /piedi|feet|foot/i.test(query);
        const urls=[];
        // 1) Google Images primo tentativo
        try{
            const res = await axios.get(`https://www.google.com/search?tbm=isch&q=${q}`, {
                headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36', 'Accept-Language': 'it-IT,it;q=0.9' },
                timeout: 7000
            });
            const html = String(res.data||'');
            // Google mette le URL in "ou":"https://..."
            const re = /"ou":"(https?:\/\/[^"]+)"/gi;
            let m;
            while((m=re.exec(html)) && urls.length<limit*2){
                let u=m[1].replace(/\\u0026/g,'&').replace(/\\/g,'');
                // Verifica piedi: se query piedi, accetta solo url che sembrano piedi o non filtra troppo
                if(isFeet){
                    // Accetta se url contiene feet/foot/sole oppure se non contiene gallery generica
                    if(/foot|feet|sole|toe|barefoot/i.test(u.toLowerCase()) || urls.length===0) {
            ...
            console.log(`[VEXAI] google found ${urls.length} for "${query}"`);
        }catch(e){ console.error('[VEXAI] google search fail', e.message); }
        // 2) Se piedi e Google non ha trovato, usa Unsplash piedi vero
        if(isFeet && urls.length < limit){
            for(let i=urls.length;i<limit;i++){
                urls.push(`https://source.unsplash.com/600x800/?feet,foot,sole,barefoot&sig=${Date.now()+i}`);
            }
        }
        // 3) Fallback generico Unsplash
        while(urls.length < limit){
            urls.push(`https://source.unsplash.com/600x400/?${q}&sig=${Date.now()+urls.length}`);
        }
        // 4) Verifica con AI se possibile: se è piedi, prova a validare via HEAD che sia immagine
        const verified=[];
        for(const u of urls){
            try{
                const head = await axios.head(u, { timeout:4000, headers:{'User-Agent':'Mozilla/5.0'} }).catch(()=>null);
                const ct = head?.headers?.['content-type']||'';
                if(ct.startsWith('image/') || u.match(/\.(jpg|jpeg|png|webp)(\?|$)/i)) verified.push(u);
                else verified.push(u); // se non verificabile, manda comunque
                if(verified.length>=limit) break;
            }catch(_){ verified.push(u); if(verified.length>=limit) break; }
        }
...
const canSendImages = async (sock, jid, urls, caption) => {
    for(const url of urls){
        ...
        for(let attempt=1; attempt<=3 && !done; attempt++){
            try{
                const res=await axios.get(url, { responseType:'arraybuffer', timeout:8000, headers:{ 'User-Agent':'VexBot/1.0' } });
                ...
                console.error(`[VEXAI] image ${attempt}/3 fail ${code||e.message} ${url.slice(0,40)}`);
                if(code===503 && attempt<3) await new Promise(r=>setTimeout(r,1200*attempt));
                else if(attempt===3){
                    // Fallback a picsum diverso
                    try{
                        const fb=`https://picsum.photos/seed/fb${Date.now()}/600/800`;
                        const r2=await axios.get(fb, { responseType:'arraybuffer', timeout:8000 });
                        await sock.sendMessage(jid, { image: Buffer.from(r2.data), caption: caption+' (fallback)', mimetype: 'image/jpeg' });
                        done=true;
                    }catch(_){}
                }
            }
...
${ownerLine}`.trim();
    ...
    // ── STATO LIVE DEL BOT (database + impostazioni, se passato) ────────────
    let snapshotBlock = '';
    try {
        const snap = String(context.botSnapshot || '').trim().slice(0, 3500);
        if (snap) snapshotBlock = `\n\nSTATO LIVE DEL BOT (dati veri, usali per rispondere sul serio):\n${snap}`;
    } catch (_) {}
    const { EXEC_HELP } = require('./ai-context');
    const systemPromptFull = `${systemPrompt}${snapshotBlock}\n\n${EXEC_HELP}`;
...
Istruzioni: rispondi come Vex AI tenendo conto di cronologia e profilo. Se il messaggio è una domanda, rispondi davvero. Se è chiacchiera, sii presente e brillante.`.trim();
    ...
    let reply = null;
    let apiError = null;
    // Prova OpenRouter, se fallisce prova Groq/Gemini fallback gratis
    const tryAsk = async (model) => {
        try {
            const altServices = { ...services, AI_MODEL: model || services.AI_MODEL };
            return await askAI({ services: altServices, system: systemPromptFull, user: userPrompt, maxTokens: 500 });
        } catch(e){ apiError = e?.response?.data?.error?.message || e?.message || String(e); console.error('[VEXAI] askAI',model,'throw:',apiError); return null; }
    };
    reply = await tryAsk();
    if(!reply && /401|auth|key|model/i.test(apiError||'')){
        // Fallback a modelli gratis noti
        const fallbacks = ['openai/gpt-3.5-turbo','google/gemini-flash-1.5','meta-llama/llama-3.1-8b-instruct:free'];
        for(const m of fallbacks){
            if(m===services.AI_MODEL) continue;
            reply = await tryAsk(m);
            if(reply) break;
        }
    }

    if (!reply) {
        // Se non va, non fare nulla (richiesta utente)
        console.log(`[VEXAI] no reply per ${displayName} — apiError=${apiError?.slice(0,60) || 'no key'}`);
        // Salva comunque la cronologia utente ma non inviare nulla
        return null;
    }
