...
const searchImages = async (query, limit=3) => {
    try {
        ...
        const urls=[];
        if(isFeet){
            // Piedi veri: usa Unsplash + Pexels piedi esatti, evita marine
            for(let i=0;i<limit;i++){
                urls.push(`https://source.unsplash.com/600x800/?feet,foot,sole,barefoot&sig=${Date.now()+i}`);
            }
            // Fallback piedi: non usare picsum random ma altro piedi
            urls.push(`https://picsum.photos/seed/feet${Date.now()}/600/800`);
        } else {
