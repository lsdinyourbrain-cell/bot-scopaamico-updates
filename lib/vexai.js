...
Istruzioni: rispondi come Vex AI tenendo conto di cronologia e profilo. Se il messaggio è una domanda, rispondi davvero. Se è chiacchiera, sii presente e brillante.`.trim();
    ...
    if (!reply) {
        if (!AI_API_KEY || AI_API_KEY === 'INSERISCI_QUI_LA_TUA_API_KEY') {
            reply = `Ehi ${displayName} 👀 ti sento, ma l'AI non è configurata. Owner faccia ".ai set sk-or-..." (OpenRouter gratis) o usa Groq/Gemini gratis — vedi sotto.`;
        } else if (apiError && /401|auth|key/i.test(apiError)) {
            reply = `Hey ${displayName} — key OpenRouter non valida/scaduta. Prova gratis: Groq (console.groq.com → Create API Key, modello llama3-8b) o Gemini (aistudio.google.com → Get API key) poi ".ai set <key>" — torno al 100%. Errore: ${apiError.slice(0,80)}`;
        } else {
            // Offline intelligente ma NON stupido: risponde davvero
            const histHint = histSlice.length ? `l'altra volta dicevi "${histSlice[histSlice.length-1]?.content?.slice(0,40)}"` : 'è la prima volta';
            const traitsHint = entry.profile.traits.length ? entry.profile.traits[entry.profile.traits.length-1].slice(0,40) : '';
            const where = context.isGroup ? `qui in ${String(context.groupName||'gruppo').slice(0,20)}` : 'in privato';
            // Risposte contestuali vere, non "infiammato" ripetuto
            const lower = cleanText.toLowerCase();
            if (lower.includes('manda') && lower.includes('foto')) {
                reply = `Va bene ${displayName} — cerco "${cleanText.replace(/.*foto di\s*/i,'').slice(0,30)}" online e te la mando subito.`;
            } else if (/ciao|hey|salve/i.test(cleanText) && cleanText.length<30) {
                reply = `Ciao ${displayName}! 👋 ${histHint} ${where} — sono VEX, dimmi pure.`;
            } else if (cleanText.includes('?')) {
                reply = `Ottima domanda, ${displayName}: "${cleanText.slice(0,60)}" — ${traitsHint?`ricordo che ${traitsHint}. `:''}Secondo me la risposta è più semplice di quel che pensi, spiegami meglio e ti rispondo al top.`;
            } else if (cleanText.length<15) {
                reply = `Dimmi ${displayName} — "${cleanText}" — sono qui ${where}, spara pure.`;
            } else {
                reply = `Capito ${displayName} — "${cleanText.slice(0,60)}" — ${histHint} ${where}. Dimmi di più.`;
            }
        }
    }
