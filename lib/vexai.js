...
// ── IMAGE SEARCH & SEND (Google accurato) ────────────────────────────────────
const searchImages = async (query, limit=3) => {
    try {
        // Google Images via scraping (più accurato di Unsplash)
        ...
        try{
            ...
            const re = /"ou":"(https?:\/\/[^"]+\.(?:jpg|jpeg|png|webp))"/gi;
            let m;
            while((m=re.exec(html)) && urls.length<limit){
                let u=m[1];
                // Filtra piedi se richiesto, altrimenti prendi tutto
                if(/piedi|feet|foot/i.test(query) && !/foot|feet|sole|toe/i.test(u.toLowerCase())) continue;
                if(!urls.includes(u)) urls.push(u);
            }
        }catch(e){ console.error('[VEXAI] google search fail', e.message); }
        // Fallback Unsplash se Google non trova abbastanza
        while(urls.length < limit){
            const isFeet = /piedi|feet|foot/i.test(query);
            if(isFeet) urls.push(`https://source.unsplash.com/600x800/?feet,foot,sole,barefoot&sig=${Date.now()+urls.length}`);
            else urls.push(`https://source.unsplash.com/600x400/?${q}&sig=${Date.now()+urls.length}`);
