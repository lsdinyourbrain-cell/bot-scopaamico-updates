...
const searchImages = async (query, limit=3) => {
    try {
        // Usa Picsum + Unsplash source come fallback senza key, e waifu per NSFW
        const isFeet = /piedi|feet|foot/i.test(query);
        const urls=[];
        if(isFeet){
            // piedi: usa picsum con seed feet + waifu
            for(let i=0;i<limit;i++){
                urls.push(`https://picsum.photos/seed/feet${Date.now()+i}/600/800`);
            }
            // Prova anche waifu.pics NSFW se disponibile
            try{
                const r=await axios.get('https://api.waifu.pics/many/nsfw/waifu', { timeout:4000 }).catch(()=>null);
                if(r?.data?.files) urls.push(...r.data.files.slice(0,1));
            }catch(_){}
        } else {
            // Generico: picsum
            for(let i=0;i<limit;i++) urls.push(`https://picsum.photos/seed/${encodeURIComponent(query)}${i}/600/400`);
        }
