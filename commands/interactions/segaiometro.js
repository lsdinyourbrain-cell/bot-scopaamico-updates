...
function getVerdict(p) {
    if (p >= 90) return '🍆✋ SEGAIOLO PRO! Callo leggendario! 🏆';
    if (p >= 70) return '🔥 Mani allenatissime!';
    if (p >= 50) return '✋ Nella media, niente male.';
    if (p >= 30) return '🤲 Poco, mani pulite.';
    if (p >= 0) return '👼 Mani sante!';
...
module.exports = {
    name: 'segaiometro',
    aliases: [],
    description: 'Misura goliardica ironica, solo per ridere.',
    async run(sock, msg, args, context) {
        ...
        const percent = hashPercent(targetJid, 'segaiometro');
        const bar = buildBar(percent);
        const verdict = getVerdict(percent);
        const tag = '@' + targetJid.split('@')[0];
        const title = toSansBold('SEGAIOMETRO');
        const emoji = '🍆';
        const line = '\u2501'.repeat(18);
        const text = `${emoji}  ${title}  ${emoji}\n${line}\n\u25b8 Utente: ${tag}\n\u25b8 Valore: *${percent}%*\n\u25b8 ${bar} ${percent}%\n${line}\n${verdict}\n${line}\n\u25c8 _Vex Bot_ \u2014 segaiometro`;
        const buttons = [
            { label: '\uD83D\uDD04 Ricalcola', id: 'segaiometro' },
            { label: '\uD83D\uDC65 Altro utente', id: 'segaiometro' }
        ];
        try {
