...
function getVerdict(p) {
    if (p >= 90) return '💀 CRINGE SUPREMO! Sei un meme vivente involontario! 🤦';
    if (p >= 70) return '😬 Abbastanza cringe, si sente.';
    if (p >= 50) return '🫣 Cringe medio, salvabile.';
    if (p >= 30) return '😌 Poco cringe, quasi safe.';
    if (p >= 0) return '✨ ZERO CRINGE! Sei smooth! 😎';
...
module.exports = {
    name: 'cringeometro',
    aliases: [],
    description: 'Misura il cringe — quanto fai rabbrividire.',
    async run(sock, msg, args, context) {
        ...
        const percent = hashPercent(targetJid, 'cringeometro');
        const bar = buildBar(percent);
        const verdict = getVerdict(percent);
        const tag = '@' + targetJid.split('@')[0];
        const title = toSansBold('CRINGEOMETRO');
        const emoji = '💀';
        const line = '\u2501'.repeat(18);
        const text = `${emoji}  ${title}  ${emoji}\n${line}\n\u25b8 Utente: ${tag}\n\u25b8 Valore: *${percent}%*\n\u25b8 ${bar} ${percent}%\n${line}\n${verdict}\n${line}\n\u25c8 _Vex Bot_ \u2014 cringeometro`;
        const buttons = [
            { label: '\uD83D\uDD04 Ricalcola', id: 'cringeometro' },
            { label: '\uD83D\uDC65 Altro utente', id: 'cringeometro' }
        ];
        try {
