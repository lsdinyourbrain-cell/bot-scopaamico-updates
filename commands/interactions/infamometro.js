...
function getVerdict(p) {
    if (p >= 90) return '🚨 INFAME SUPREMO! Vendresti anche tua nonna! 🤥';
    if (p >= 70) return '😒 Infame notevole.';
    if (p >= 50) return '😐 Medio infame, dipende.';
    if (p >= 30) return '😇 Poco infame, quasi bravo.';
    if (p >= 0) return '😇 SANTO! Mai infame!';
...
module.exports = {
    name: 'infamometro',
    aliases: [],
    description: 'Misura livello infame/spia — scherzoso.',
    async run(sock, msg, args, context) {
        ...
        const percent = hashPercent(targetJid, 'infamometro');
        const bar = buildBar(percent);
        const verdict = getVerdict(percent);
        const tag = '@' + targetJid.split('@')[0];
        const title = toSansBold('INFAMOMETRO');
        const emoji = '🤥';
        const line = '\u2501'.repeat(18);
        const text = `${emoji}  ${title}  ${emoji}\n${line}\n\u25b8 Utente: ${tag}\n\u25b8 Valore: *${percent}%*\n\u25b8 ${bar} ${percent}%\n${line}\n${verdict}\n${line}\n\u25c8 _Vex Bot_ \u2014 infamometro`;
        const buttons = [
            { label: '\uD83D\uDD04 Ricalcola', id: 'infamometro' },
            { label: '\uD83D\uDC65 Altro utente', id: 'infamometro' }
        ];
        try {
