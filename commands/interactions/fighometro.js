...
function getVerdict(p) {
    if (p >= 90) return '😎🔥 FIGHISSIMO! Sei un modello vivente! ✨';
    if (p >= 70) return '😏 Molto figo, spacchi!';
    if (p >= 50) return '🙂 Figo nella media.';
    if (p >= 30) return '🥲 Un po\' da rivedere.';
    if (p >= 0) return '🥔 Patata al momento, ma puoi migliorare!';
...
module.exports = {
    name: 'fighometro',
    aliases: [],
    description: 'Misura quanto sei figo — coolness level.',
    async run(sock, msg, args, context) {
        ...
        const percent = hashPercent(targetJid, 'fighometro');
        const bar = buildBar(percent);
        const verdict = getVerdict(percent);
        const tag = '@' + targetJid.split('@')[0];
        const title = toSansBold('FIGHOMETRO');
        const emoji = '😎';
        const line = '\u2501'.repeat(18);
        const text = `${emoji}  ${title}  ${emoji}\n${line}\n\u25b8 Utente: ${tag}\n\u25b8 Valore: *${percent}%*\n\u25b8 ${bar} ${percent}%\n${line}\n${verdict}\n${line}\n\u25c8 _Vex Bot_ \u2014 fighometro`;
        const buttons = [
            { label: '\uD83D\uDD04 Ricalcola', id: 'fighometro' },
            { label: '\uD83D\uDC65 Altro utente', id: 'fighometro' }
        ];
        try {
