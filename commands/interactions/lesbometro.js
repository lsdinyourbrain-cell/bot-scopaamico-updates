...
function getVerdict(p) {
    if (p >= 90) return '👩‍❤️‍👩 ICONA LESBO! Sei bandiera vivente! 🏳️‍🌈🔥';
    if (p >= 70) return '💘 Fortissima energia sapphica!';
    if (p >= 50) return '🌸 Dolce e bilanciata, come peonia.';
    if (p >= 30) return '🍃 Un filo, ma discreto.';
    if (p >= 0) return '🪵 Zero vibes, tutta altra strada.';
...
module.exports = {
    name: 'lesbometro',
    aliases: ["lesbmeter"],
    description: 'Misura il vibe lesbo con stile e ironia.',
    async run(sock, msg, args, context) {
        ...
        const percent = hashPercent(targetJid, 'lesbometro');
        const bar = buildBar(percent);
        const verdict = getVerdict(percent);
        const tag = '@' + targetJid.split('@')[0];
        const title = toSansBold('LESBOMETRO');
        const emoji = '💖';
        const line = '\u2501'.repeat(18);
        const text = `${emoji}  ${title}  ${emoji}\n${line}\n\u25b8 Utente: ${tag}\n\u25b8 Valore: *${percent}%*\n\u25b8 ${bar} ${percent}%\n${line}\n${verdict}\n${line}\n\u25c8 _Vex Bot_ \u2014 lesbometro`;
        const buttons = [
            { label: '\uD83D\uDD04 Ricalcola', id: 'lesbometro' },
            { label: '\uD83D\uDC65 Altro utente', id: 'lesbometro' }
        ];
        try {
