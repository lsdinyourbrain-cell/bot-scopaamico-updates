...
function getVerdict(p) {
    if (p >= 90) return '😈 PERVERTITO LEGGENDARIO! Hai pensieri 24/7! 🔥';
    if (p >= 70) return '🥵 Pervertito forte, occhio birichino!';
    if (p >= 50) return '😏 A meta\', mente piccantina.';
    if (p >= 30) return '😇 Quasi santo, pochi pensieri zozzi.';
    if (p >= 0) return '👼 Puro come acqua di sorgente!';
...
module.exports = {
    name: 'pervertometro',
    aliases: ["pervometro"],
    description: 'Misura il tasso pervertito ironico.',
    async run(sock, msg, args, context) {
        ...
        const percent = hashPercent(targetJid, 'pervertometro');
        const bar = buildBar(percent);
        const verdict = getVerdict(percent);
        const tag = '@' + targetJid.split('@')[0];
        const title = toSansBold('PERVERTOMETRO');
        const emoji = '😏';
        const line = '\u2501'.repeat(18);
        const text = `${emoji}  ${title}  ${emoji}\n${line}\n\u25b8 Utente: ${tag}\n\u25b8 Valore: *${percent}%*\n\u25b8 ${bar} ${percent}%\n${line}\n${verdict}\n${line}\n\u25c8 _Vex Bot_ \u2014 pervertometro`;
        const buttons = [
            { label: '\uD83D\uDD04 Ricalcola', id: 'pervertometro' },
            { label: '\uD83D\uDC65 Altro utente', id: 'pervertometro' }
        ];
        try {
