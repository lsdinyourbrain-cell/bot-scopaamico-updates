...
function getVerdict(p) {
    if (p >= 90) return '👠💋 ZOCCOLA ICONICA! Tacchi a spillo infuocati! 🔥';
    if (p >= 70) return '💃 Zoccolaggine elevata, passerella tua!';
    if (p >= 50) return '😏 Un po\' zoccola, un po\' santa.';
    if (p >= 30) return '🙈 Poco zoccola, piu\' acqua e sapone.';
    if (p >= 0) return '👼 Santa subito! Zero zoccolate!';
...
module.exports = {
    name: 'zoccolometro',
    aliases: [],
    description: 'Misura ironica piccante — solo per ridere.',
    async run(sock, msg, args, context) {
        ...
        const percent = hashPercent(targetJid, 'zoccolometro');
        const bar = buildBar(percent);
        const verdict = getVerdict(percent);
        const tag = '@' + targetJid.split('@')[0];
        const title = toSansBold('ZOCCOLOMETRO');
        const emoji = '👠';
        const line = '\u2501'.repeat(18);
        const text = `${emoji}  ${title}  ${emoji}\n${line}\n\u25b8 Utente: ${tag}\n\u25b8 Valore: *${percent}%*\n\u25b8 ${bar} ${percent}%\n${line}\n${verdict}\n${line}\n\u25c8 _Vex Bot_ \u2014 zoccolometro`;
        const buttons = [
            { label: '\uD83D\uDD04 Ricalcola', id: 'zoccolometro' },
            { label: '\uD83D\uDC65 Altro utente', id: 'zoccolometro' }
        ];
        try {
