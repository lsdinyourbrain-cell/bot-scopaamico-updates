...
function getVerdict(p) {
    if (p >= 90) return '🐉🔥 DRAGO LEGGENDARIO! Sputi fuoco ovunque!';
    if (p >= 70) return '🐲 Drago potente!';
    if (p >= 50) return '🦎 Draghetto in crescita.';
    if (p >= 30) return '🥚 Uovo di drago, devi schiuderti.';
    if (p >= 0) return '🐔 Piu\' pollo che drago!';
...
module.exports = {
    name: 'dragoometro',
    aliases: [],
    description: 'Misura quanto sei drago — potenza e fuoco!',
    async run(sock, msg, args, context) {
        ...
        const percent = hashPercent(targetJid, 'dragoometro');
        const bar = buildBar(percent);
        const verdict = getVerdict(percent);
        const tag = '@' + targetJid.split('@')[0];
        const title = toSansBold('DRAGOOMETRO');
        const emoji = '🐉';
        const line = '\u2501'.repeat(18);
        const text = `${emoji}  ${title}  ${emoji}\n${line}\n\u25b8 Utente: ${tag}\n\u25b8 Valore: *${percent}%*\n\u25b8 ${bar} ${percent}%\n${line}\n${verdict}\n${line}\n\u25c8 _Vex Bot_ \u2014 dragoometro`;
        const buttons = [
            { label: '\uD83D\uDD04 Ricalcola', id: 'dragoometro' },
            { label: '\uD83D\uDC65 Altro utente', id: 'dragoometro' }
        ];
        try {
