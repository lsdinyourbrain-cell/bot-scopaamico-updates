...
function getVerdict(p) {
    if (p >= 90) return '💑 ETERO AL 100%! Coppia da spot matrimoniale! 👰🤵';
    if (p >= 70) return '😍 Molto etero, profumo di colonia classica.';
    if (p >= 50) return '⚖️ Meta\' e meta\', fluid vibes.';
    if (p >= 30) return '🌈 Un pizzico arcobaleno non guasta.';
    if (p >= 0) return '🦄 Totalmente rainbow!';
...
module.exports = {
    name: 'eterometro',
    aliases: [],
    description: 'Misura quanto sei etero — puro divertimento.',
    async run(sock, msg, args, context) {
        ...
        const percent = hashPercent(targetJid, 'eterometro');
        const bar = buildBar(percent);
        const verdict = getVerdict(percent);
        const tag = '@' + targetJid.split('@')[0];
        const title = toSansBold('ETEROMETRO');
        const emoji = '💑';
        const line = '\u2501'.repeat(18);
        const text = `${emoji}  ${title}  ${emoji}\n${line}\n\u25b8 Utente: ${tag}\n\u25b8 Valore: *${percent}%*\n\u25b8 ${bar} ${percent}%\n${line}\n${verdict}\n${line}\n\u25c8 _Vex Bot_ \u2014 eterometro`;
        const buttons = [
            { label: '\uD83D\uDD04 Ricalcola', id: 'eterometro' },
            { label: '\uD83D\uDC65 Altro utente', id: 'eterometro' }
        ];
        try {
