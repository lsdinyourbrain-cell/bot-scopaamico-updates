...
function getVerdict(p) {
    if (p >= 90) return '🥜🤡 COGLIONE GALATTICO! Sei il re dei coglioni! 👑';
    if (p >= 70) return '😵 Coglione di alto livello!';
    if (p >= 50) return '😅 Medio coglione, simpatico.';
    if (p >= 30) return '🙂 Poco coglione, quasi serio.';
    if (p >= 0) return '🧠 SVEGLIO! Zero coglionaggine!';
...
module.exports = {
    name: 'coglionometro',
    aliases: [],
    description: 'Misura livello coglione — bonario e scherzoso.',
    async run(sock, msg, args, context) {
        ...
        const percent = hashPercent(targetJid, 'coglionometro');
        const bar = buildBar(percent);
        const verdict = getVerdict(percent);
        const tag = '@' + targetJid.split('@')[0];
        const title = toSansBold('COGLIONOMETRO');
        const emoji = '🥜';
        const line = '\u2501'.repeat(18);
        const text = `${emoji}  ${title}  ${emoji}\n${line}\n\u25b8 Utente: ${tag}\n\u25b8 Valore: *${percent}%*\n\u25b8 ${bar} ${percent}%\n${line}\n${verdict}\n${line}\n\u25c8 _Vex Bot_ \u2014 coglionometro`;
        const buttons = [
            { label: '\uD83D\uDD04 Ricalcola', id: 'coglionometro' },
            { label: '\uD83D\uDC65 Altro utente', id: 'coglionometro' }
        ];
        try {
