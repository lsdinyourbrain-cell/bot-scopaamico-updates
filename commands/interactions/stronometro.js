...
function getVerdict(p) {
    if (p >= 90) return '💩👑 STRONZO SUPREMO! Hai il trono di stronzi! 🚽';
    if (p >= 70) return '😒 Stronzo notevole!';
    if (p >= 50) return '😐 Medio stronzo, dipende dal giorno.';
    if (p >= 30) return '😇 Poco stronzo, quasi angelo.';
    if (p >= 0) return '😇 CUORE D\'ORO! Zero stronzezza!';
...
module.exports = {
    name: 'stronometro',
    aliases: [],
    description: 'Misura quanto sei stronzo — bonariamente.',
    async run(sock, msg, args, context) {
        ...
        const percent = hashPercent(targetJid, 'stronometro');
        const bar = buildBar(percent);
        const verdict = getVerdict(percent);
        const tag = '@' + targetJid.split('@')[0];
        const title = toSansBold('STRONOMETRO');
        const emoji = '💩';
        const line = '\u2501'.repeat(18);
        const text = `${emoji}  ${title}  ${emoji}\n${line}\n\u25b8 Utente: ${tag}\n\u25b8 Valore: *${percent}%*\n\u25b8 ${bar} ${percent}%\n${line}\n${verdict}\n${line}\n\u25c8 _Vex Bot_ \u2014 stronometro`;
        const buttons = [
            { label: '\uD83D\uDD04 Ricalcola', id: 'stronometro' },
            { label: '\uD83D\uDC65 Altro utente', id: 'stronometro' }
        ];
        try {
