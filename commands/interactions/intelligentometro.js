...
function getVerdict(p) {
    if (p >= 90) return '🧠💡 GENIO! QI da Nobel! 🏆';
    if (p >= 70) return '📚 Molto intelligente, cervellone!';
    if (p >= 50) return '🙂 Nella media, furbo!';
    if (p >= 30) return '🥴 Un po\' lento, studia!';
    if (p >= 0) return '🦆 Cervello in modalita\' aereo!';
    return '✨ Risultato unico!';
}

module.exports = {
    name: 'intelligentometro',
    aliases: [],
    description: 'Misura l\'intelligenza in modo goliardico.',
    async run(sock, msg, args, context) {
        ...
        const title = toSansBold('INTELLIGENTOMETRO');
        const emoji = '🧠';
        const line = '\u2501'.repeat(18);
        const text = `${emoji}  ${title}  ${emoji}\n${line}\n\u25b8 Utente: ${tag}\n\u25b8 Valore: *${percent}%*\n\u25b8 ${bar} ${percent}%\n${line}\n${verdict}\n${line}\n\u25c8 _Vex Bot_ \u2014 intelligentometro`;
        const buttons = [
            { label: '\uD83D\uDD04 Ricalcola', id: 'intelligentometro' },
            { label: '\uD83D\uDC65 Altro utente', id: 'intelligentometro' }
        ];
        try {
