...
function getVerdict(p) {
    if (p >= 90) return '🛏️🔥 SCOPATORE SERIALE! Materasso in fiamme!';
    if (p >= 70) return '❤️‍🔥 Scopi parecchio!';
    if (p >= 50) return '😏 Nella media hot.';
    if (p >= 30) return '🥲 Poco movimento sotto le coperte.';
    if (p >= 0) return '🛌 Letto a prova di polvere!';
...
module.exports = {
    name: 'scopometro',
    aliases: [],
    description: 'Misura ironica piccante quante scopate (gioco).',
    async run(sock, msg, args, context) {
        ...
        const percent = hashPercent(targetJid, 'scopometro');
        const bar = buildBar(percent);
        const verdict = getVerdict(percent);
        const tag = '@' + targetJid.split('@')[0];
        const title = toSansBold('SCOPOMETRO');
        const emoji = '🛏️';
        const line = '\u2501'.repeat(18);
        const text = `${emoji}  ${title}  ${emoji}\n${line}\n\u25b8 Utente: ${tag}\n\u25b8 Valore: *${percent}%*\n\u25b8 ${bar} ${percent}%\n${line}\n${verdict}\n${line}\n\u25c8 _Vex Bot_ \u2014 scopometro`;
        const buttons = [
            { label: '\uD83D\uDD04 Ricalcola', id: 'scopometro' },
            { label: '\uD83D\uDC65 Altro utente', id: 'scopometro' }
        ];
        try {
