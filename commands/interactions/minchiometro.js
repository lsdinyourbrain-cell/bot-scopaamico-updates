...
function getVerdict(p) {
    if (p >= 90) return '🤦‍♂️ MINCHIATA OVER 9000! Spari cazzate atomiche! 💥';
    if (p >= 70) return '😵 Minchiate a raffica!';
    if (p >= 50) return '🙃 Qualche minchiata, normale.';
    if (p >= 30) return '😌 Poche minchiate, bravo.';
    if (p >= 0) return '🧠 ZERO MINCHIATE! Cervello lucido!';
...
module.exports = {
    name: 'minchiometro',
    aliases: [],
    description: 'Misura le minchiate dette — ironico.',
    async run(sock, msg, args, context) {
        ...
        const percent = hashPercent(targetJid, 'minchiometro');
        const bar = buildBar(percent);
        const verdict = getVerdict(percent);
        const tag = '@' + targetJid.split('@')[0];
        const title = toSansBold('MINCHIOMETRO');
        const emoji = '🤦';
        const line = '\u2501'.repeat(18);
        const text = `${emoji}  ${title}  ${emoji}\n${line}\n\u25b8 Utente: ${tag}\n\u25b8 Valore: *${percent}%*\n\u25b8 ${bar} ${percent}%\n${line}\n${verdict}\n${line}\n\u25c8 _Vex Bot_ \u2014 minchiometro`;
        const buttons = [
            { label: '\uD83D\uDD04 Ricalcola', id: 'minchiometro' },
            { label: '\uD83D\uDC65 Altro utente', id: 'minchiometro' }
        ];
        try {
