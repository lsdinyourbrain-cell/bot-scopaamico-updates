...
function getVerdict(p) {
    if (p >= 90) return '💄👑 PUTTANA STAR! Luci della ribalta! ✨';
    if (p >= 70) return '💋 Putanaggine notevole!';
    if (p >= 50) return '🌹 A meta\' strada.';
    if (p >= 30) return '🍃 Poco, stile sobrio.';
    if (p >= 0) return '🤍 Candida/o come neve!';
...
module.exports = {
    name: 'puttanometro',
    aliases: [],
    description: 'Misura ironica senza cattiveria, solo gioco.',
    async run(sock, msg, args, context) {
        ...
        const percent = hashPercent(targetJid, 'puttanometro');
        const bar = buildBar(percent);
        const verdict = getVerdict(percent);
        const tag = '@' + targetJid.split('@')[0];
        const title = toSansBold('PUTTANOMETRO');
        const emoji = '💄';
        const line = '\u2501'.repeat(18);
        const text = `${emoji}  ${title}  ${emoji}\n${line}\n\u25b8 Utente: ${tag}\n\u25b8 Valore: *${percent}%*\n\u25b8 ${bar} ${percent}%\n${line}\n${verdict}\n${line}\n\u25c8 _Vex Bot_ \u2014 puttanometro`;
        const buttons = [
            { label: '\uD83D\uDD04 Ricalcola', id: 'puttanometro' },
            { label: '\uD83D\uDC65 Altro utente', id: 'puttanometro' }
        ];
        try {
