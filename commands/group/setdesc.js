...
module.exports = {
    ...
    async run(sock, msg, args, context) {
        ...
        if (!isSenderAdmin) return reply("Solo gli admin possono cambiare la descrizione.");
        if (!isBotAdmin) return reply("Rendimi admin prima.");
        if (!textArgs) return reply("Inserisci la nuova descrizione.");

        await sock.groupUpdateDescription(from, textArgs);
        await reply(`✅ Descrizione aggiornata.`);
    },
};
